...
const STAGE_LINEAR = {
  ...
  quiz: [
    {
      q: '順番がバラバラな1万人の名簿から特定の1人を線形探索で探すとき、最悪の場合の確認回数は？',
      choices: ['約100回', '1万回', '100万回'],
      correct: 1,
      explain: '線形探索は端から順に確認するので、最悪の場合は人数と同じ回数かかります（O(N)）。',
    },
    {
      q: '線形探索の強みは？',
      choices: ['データがどんな順番でも使える', '名簿が並んでいるときだけ速くなる', '確認回数が常に1回で済む'],
      correct: 0,
      explain: '並び順に関係なく必ず使えるのが強み。そのかわり、人数に比例して遅くなります。',
    },
  ],
...
const STAGE_BINARY = {
  ...
  quiz: [
    {
      q: '五十音順に並んだ1,000人の名簿を二分探索するとき、最悪でもおよそ何回の比較で見つかる？',
      choices: ['約10回', '約100回', '約500回'],
      correct: 0,
      explain: '半分ずつ絞り込むので、2の10乗=1024 ≧ 1,000 より約10回で済みます（O(log N)）。',
    },
    {
      q: '二分探索を使うために必要な条件は？',
      choices: ['データが順番に並んでいること', '人数が偶数であること', 'データが100件以下であること'],
      correct: 0,
      explain: '「中央と比べて半分を捨てる」には、データが整列していることが前提です。',
    },
  ],
...
const STAGE_BUBBLE = {
  ...
  quiz: [
    {
      q: 'バブルソートで荷物の数が100個から200個（2倍）になると、比較の手間はおよそ何倍になる？',
      choices: ['約2倍', '約4倍', 'ほぼ変わらない'],
      correct: 1,
      explain: '隣同士の比較はおよそN×N回。Nが2倍なら手間は約4倍です（O(N²)）。',
    },
    {
      q: '1回のパス（端から端まで比較して交換）で必ず起きることは？',
      choices: ['一番重い荷物が右端に確定する', '全体が完全に整列する', '一番軽い荷物が消える'],
      correct: 0,
      explain: '泡（バブル）が浮かぶように、パスごとに最大の荷物が端へ確定していきます。',
    },
  ],
...
const STAGE_QUICK = {
  ...
  quiz: [
    {
      q: 'クイックソートで基準（ピボット）を選んだ後、最初にすることは？',
      choices: ['基準より軽いか重いかで荷物を左右に仕分ける', '隣同士を順番に交換していく', '端から順に数を数える'],
      correct: 0,
      explain: '基準で左右に仕分けると基準の位置が確定し、あとは左右をそれぞれ同じ方法で片付けます（分割統治）。',
    },
    {
      q: 'クイックソートの平均の計算量は？',
      choices: ['O(N log N)', 'O(N²)', 'O(N)'],
      correct: 0,
      explain: '仕分けの深さがおよそlog N段、各段で合計N回の比較なのでO(N log N)。バブルソートより桁違いに速くなります。',
    },
  ],
...
const STAGE_DIJKSTRA = {
  ...
  quiz: [
    {
      q: 'ダイクストラ法で、次に距離を「確定」させる町はどれ？',
      choices: ['まだ確定していない中で、距離が最小の町', '出発点から一番遠い町', 'つながる道の本数が一番多い町'],
      correct: 0,
      explain: '距離最小の町より近回りする道は存在しないので、安心して確定できます。これがダイクストラ法の核心です。',
    },
    {
      q: 'ダイクストラ法で求まるものは？',
      choices: ['出発点から各町への最短距離', '全ての町を一度ずつ回る最短の道順', '王国で一番交通量の多い道'],
      correct: 0,
      explain: '出発点からの最短距離を全ての町について求めます。カーナビの経路探索の基礎です。',
    },
  ],
...
const STAGE_DP = {
  ...
  quiz: [
    {
      q: '18段の階段の上り方を求めるとき、17段と16段の答えがすでに分かっていたら？',
      choices: ['2つを足すだけで求まる', 'もう一度1段目から数え直す必要がある', '2つを掛け算する'],
      correct: 0,
      explain: '18段への最後の一歩は「17段から1歩」か「16段から2歩」の2通りだけ。だから答えは2つの和になります。',
    },
    {
      q: '「メモ化」の効果として正しいものは？',
      choices: ['同じ計算を繰り返さずに済む', '答えがより正確になる', '使うメモリが必ず減る'],
      correct: 0,
      explain: '一度計算した答えを覚えて再利用するのがメモ化。動的計画法の核心で、計算回数が劇的に減ります。',
    },
  ],
